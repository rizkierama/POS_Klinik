...
<div class="flex">
    ...
    <div id="main-content" class="ml-64 p-6 transition-all duration-300 w-full">
        <div class="max-w-2xl mx-auto">
            <div class="bg-gradient-to-r from-blue-500 to-cyan-500 text-white text-center rounded-lg p-3 shadow-lg mb-2">
                <h1 class="text-3xl font-bold">Edit Data Obat</h1>
                <p class="text-sm mt-2">Silakan perbarui informasi obat dengan benar.</p>
            </div>

            <div class="bg-white p-4 rounded-lg shadow-lg">
                <form action="{{ route('obat.update', $obat) }}" method="POST" class="space-y-4">
                    @csrf
                    @method('PUT')

                    <div>
                        <label for="nama_obat" class="block mb-1 text-sm font-medium text-gray-700">Nama Obat</label>
                        <input type="text" name="nama_obat" id="nama_obat" value="{{ $obat->nama_obat }}" class="w-full border border-gray-300 p-3 rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none" required>
                    </div>

                    <div>
                        <label for="stok" class="block mb-1 text-sm font-medium text-gray-700">Stok</label>
                        <input type="number" name="stok" id="stok" value="{{ $obat->stok }}" class="w-full border border-gray-300 p-3 rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none" required>
                    </div>

                    <div>
                        <label for="harga" class="block mb-1 text-sm font-medium text-gray-700">Harga</label>
                        <input type="number" name="harga" id="harga" value="{{ $obat->harga }}" class="w-full border border-gray-300 p-3 rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none" required>
                    </div>

                    <div class="flex justify-center space-x-4">
                        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-full shadow-md transition-all">Simpan Perubahan</button>
                        <a href="{{ route('obat.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white px-6 py-2 rounded-full shadow-md transition-all">Kembali</a>
                    </div>
                </form>
            </div>
        </div>
